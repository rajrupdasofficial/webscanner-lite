...
# Output folders - create automatically if not exists
OUT_TXT_DIR = "shallow_reports_txt"
OUT_XLSX_DIR = "shallow_reports_xlsx"
OUT_PDF_DIR = "shallow_reports_pdf"

def ensure_directories():
    """Create output directories if they don't exist, skip if they exist."""
    for d in (OUT_TXT_DIR, OUT_XLSX_DIR, OUT_PDF_DIR):
        if not os.path.exists(d):
            os.makedirs(d, exist_ok=True)
            logger.info("Created directory: %s", d)
        else:
            logger.debug("Directory already exists, skipping: %s", d)
...
def download_and_install_nikto():
    ...
    try:
        ...
        with open(NIKTO_TARBALL, "wb") as f, tqdm(
            total=total, unit="B", unit_scale=True, unit_divisor=1024, desc="Downloading Nikto"
        ) as pbar:
...
def validate_url_early(target_url):
    logger.info("Validating URL: %s", target_url)
    ...
    logger.info("DNS resolution successful for %s", hostname)
    r = requests.head(target_url, timeout=10, allow_redirects=True)
    r.raise_for_status()
    logger.info("HEAD request successful for %s", target_url)
    return target_url
...
def write_excel(output_excel_file, timestamp, target_url, issues):
    ...
    logger.info("Scan results successfully saved to Excel: %s", output_excel_file)
...
def write_pdf(output_pdf_file, timestamp, target_url, issues):
    ...
    c.showPage()
    c.save()
    logger.info("Scan results successfully saved to PDF: %s", output_pdf_file)

def run_nikto_scan_single(target_url, nikto_path, timeout_sec=600):
    ...
    # Use absolute path for Nikto output to avoid relative path issues
    output_txt_abs = os.path.abspath(output_txt_file)

    logger.info("Running Nikto scan on: %s", target_url)
    logger.info("TXT: %s | XLSX: %s | PDF: %s", output_txt_abs, output_excel_file, output_pdf_file)

    # Nikto invocation with absolute output path
    if nikto_path == "nikto":
        cmd = ["nikto", "-h", target_url, "-output", output_txt_abs, "-Format", "txt"]
        cwd = None
    else:
        cwd = os.path.join(NIKTO_INSTALL_DIR, "program")
        cmd = ["perl", "nikto.pl", "-h", target_url, "-output", output_txt_abs, "-Format", "txt"]

    logger.info("Executing Nikto command: %s", " ".join(cmd))
    proc = subprocess.Popen(cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    # Live progress bar tied to timeout
    start = time.time()
    ...
    with tqdm(total=100, desc=f"Scanning {target_url}", unit="%", leave=True) as pbar:
    ...
    if returncode != 0:
        logger.error("Nikto scan failed (%s): %s", target_url, stderr)
        ...
        logger.info("Nikto scan completed for %s", target_url)

    # Parse Nikto TXT for "+ …" findings using the absolute path
...
# Top-level worker to avoid pickling errors with multiprocessing
def worker_run(args):
    target, nikto_path, timeout_sec = args
    return run_nikto_scan_single(target, nikto_path, timeout_sec)

def run_targets(targets, timeout_sec=600):
...
def main():
    ...
    targets = sys.argv[1:]
    try:
        results = run_targets(targets, timeout_sec=300)  # adjust timeout here if needed
